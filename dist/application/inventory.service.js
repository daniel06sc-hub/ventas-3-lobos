...
class InventoryService {
    ...
    constructor(beerStyleRepository, systemSettingsRepository) {
    ...
    async updateBeerStyle(id, data) {
        ...
        if (data.priceUnit !== undefined)
            style.priceUnit = Math.max(0, data.priceUnit);
        if (data.pricePack2 !== undefined)
            style.pricePack2 = Math.max(0, data.pricePack2);
        if (data.pricePack3 !== undefined)
            style.pricePack3 = Math.max(0, data.pricePack3);
        if (data.pricePack4 !== undefined)
            style.pricePack4 = Math.max(0, data.pricePack4);
        if (data.priceWholesale !== undefined)
            style.priceWholesale = Math.max(0, data.priceWholesale);
     ...
     */
    async updateWholesaleUnits(units) {
        if (!units || units <= 0) {
            throw new Error('La cantidad de unidades para formato mayorista debe ser mayor a 0');
        }
        await this.systemSettingsRepository.setVal('wholesale_units', units.toString(), 'Cantidad de unidades (botellas) que componen el Formato Mayorista');
    }
}
