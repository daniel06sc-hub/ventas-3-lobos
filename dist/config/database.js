...
async function getDatabase() {
    ...
    // Ruta física del archivo de base de datos en la raíz del proyecto
    ...
    try {
        ...
        const needsFkMigration = fkList.some((fk) => fk.on_delete === 'NO ACTION');
        if (needsFkMigration) {
    ...
    const wholesaleSetting = await dbInstance.get('SELECT * FROM system_settings WHERE key = ?', 'wholesale_units');
    if (!wholesaleSetting) {
        await dbInstance.run('INSERT INTO system_settings (key, value, description) VALUES (?, ?, ?)', 'wholesale_units', '23', 'Cantidad de unidades (botellas) que componen el Formato Mayorista para 3 Lobos');
    }
    ...
    if (userCountRow && userCountRow.count === 0) {
        ...
        const sellerHash = await bcryptjs_1.default.hash('vendedor123', 10);
        // Insertar Administrador por defecto
        await dbInstance.run('INSERT INTO users (id, username, password_hash, name, role) VALUES (?, ?, ?, ?, ?)', 'u-admin-default-id', 'admin', adminHash, 'Administrador 3 Lobos', 'admin');
        // Insertar Vendedor por defecto
        await dbInstance.run('INSERT INTO users (id, username, password_hash, name, role) VALUES (?, ?, ?, ?, ?)', 'u-vendedor-default-id', 'vendedor', sellerHash, 'Vendedor Cervecería', 'vendedor');
    ...
    if (styleCountRow && styleCountRow.count === 0) {
        ...
        for (const s of styles) {
            await dbInstance.run(`INSERT INTO beer_styles (id, name, stock_bottles, price_unit, price_pack2, price_pack3, price_pack4, price_wholesale)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, s.id, s.name, s.stock, s.p1, s.p2, s.p3, s.p4, s.pW);
        }
    ...
    if (customerCountRow && customerCountRow.count === 0) {
        await dbInstance.run(`INSERT INTO customers (id, business_name, fiscal_id, phone, customer_type)
       VALUES (?, ?, ?, ?, ?)`, 'cust-bar-lobo', 'El Bar del Lobo Solitario', '76.543.210-K', '+56912345678', 'wholesale');
        await dbInstance.run(`INSERT INTO customers (id, business_name, fiscal_id, phone, customer_type)
       VALUES (?, ?, ?, ?, ?)`, 'cust-resto-andes', 'Restaurante Cumbres Andinas', '78.987.654-3', '+56987654321', 'wholesale');
